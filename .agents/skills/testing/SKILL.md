...
> (AGENTS.md "Архитектурные решения" — "Тесты": "port 0, rcgen, serial_test для Admin API,
...
4. **Cover the fail-open/fail-closed boundary explicitly** for anything security-relevant
   (JWT, ForwardAuth, WASM, consumers) — these have documented invariants in AGENTS.md
...
- **English only** — test names, assertions messages, comments (AGENTS.md "Language").
...
- Don't write E2E/browser tests — there's no UI to test (AGENTS.md "No end-user UI to localize").
