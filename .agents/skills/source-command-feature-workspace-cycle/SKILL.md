---
name: "source-command-feature-workspace-cycle"
description: "One autonomous iteration of the Conduit 2.0 workspace-migration cycle (#114) — Dependabot triage, a periodic integrity audit of already-shipped code/tests/docs, pick up or decompose a sub-issue, implement it on a branch, self-review, get it green, update docs, merge, log a summary, and check whether"
---

# source-command-feature-workspace-cycle

Use this skill when the user asks to run the migrated source command `feature-workspace-cycle`.
...
- Skim the most recent "Реализовано в сессии" entries in `AGENTS.md` and the
...
- **Before any merge: `security-engineer` sign-off, unconditionally** — every
  ...
  `.Codex/rules/workflow.md` "Security review is unconditional"). Spawn it
  ...
  **The PASS is only valid for the head SHA it actually reviewed** — if the
  ...
  don't merge a SHA the agent never saw (see `.Codex/rules/workflow.md`).
...
- If merges to `main` accumulate to something worth shipping (several fixes,
  ...
  — but only **cut** one (`release-engineer`, `.Codex/skills/release/
...
- This step **is** the daily instance of the "Dependabot & branch hygiene
  reflex check" (`.Codex/rules/index.md`) — also list all branches and
  ...
  Log the outcome as a row in `AGENTS.md`'s "Dependabot & branch hygiene
...
- **Keep the migration branch in sync with `main`**: every merge in this step
  moves `main` independently of `Codex/cargo-workspace-features-23qxfr` —
...
- Pick one feature/module/config area not audited recently (check the audit
  log's own entries first; failing that, `AGENTS.md`'s oldest "Реализовано"
...
- For each gap it reports, route by **risk and ambiguity**, not by which
  ...
  - If the auditor flags that a gap looks like "missing a known-good
    ...
    **`prior-art-researcher`** before deciding the fix — see the note in
    Step 2 below.
- Log what you audited and found (even "no gaps") in `AGENTS.md`'s
...
- If the task genuinely needs "how do others solve this" input before you can
  ...
  `ReplayBody`, h2o/Angie/Envoy/HAProxy's algorithms — see `AGENTS.md`'s own
...
- Create a branch off the current tip of the 2.0 migration branch
  (`Codex/cargo-workspace-features-23qxfr`), never off `main` directly for
  #114 work.
...
- Open the PR against `Codex/cargo-workspace-features-23qxfr` (draft is
...
- Review your own diff as if it were someone else's: correctness, scope
  creep, missed edge cases, anything that contradicts `AGENTS.md`'s recorded
...
- Once green and reviewed, merge the PR into
  `Codex/cargo-workspace-features-23qxfr` (call **`release-engineer`** first
...
- Same unconditional gate as Step 1: **`security-engineer` sign-off before
  ...
  tracking PR (`Codex/cargo-workspace-features-23qxfr` → `main`, e.g. #152)
...
- If #114 is fully, elegantly, and concisely done — every planned crate
  ...
  issue. In order:
  1. Get the tracking PR (`Codex/cargo-workspace-features-23qxfr` → `main`,
     ...
     merge (Step 7).
  2. Call **`release-engineer`** and follow `.Codex/skills/release/SKILL.md`
