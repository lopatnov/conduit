---
name: "source-command-dependabot-hygiene"
...
# source-command-dependabot-hygiene

Use this skill when the user asks to run the migrated source command `dependabot-hygiene`.

## Command Template

# /dependabot-hygiene — Dependabot & branch hygiene reflex check

> Extracted 2026-08-28 from `.Codex/rules/index.md` "Dependabot & branch hygiene reflex
> check" (see that file's "New `.Codex/` process content: command/skill by default, not
...
sweep has run in the last ~24h (see `.Codex/logs/dependabot-hygiene.md`). If the newest
...
- List all branches and cross-reference against PRs in every state. A branch with **no
  ...
  push --delete` is blocked from inside a Codex session by this environment's git
...
- **Recommend the user enable "Automatically delete head branches"** (repo Settings →
  ...
  action.
- Log the result — even "nothing new" — as a new row in `.Codex/logs/dependabot-hygiene.md`,
