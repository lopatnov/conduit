...
user. Assume the reader has: the repo's `AGENTS.md` and `.Codex/` (read fresh, so don't restate
...
5. **Scheduled/background work still pending** — any `ScheduleWakeup`, `send_later`, or Routine
   ...
   - **First check — on the actual branch/ref this work is based on, not just any branch of the
     repo — whether `.Codex/commands/session-rotate.md` already exists** (`git show` or
...
code") that's already in `AGENTS.md`/`.Codex/rules/`.
