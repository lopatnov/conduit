...
1. **Stray worktrees**: `git worktree list`. For each entry that isn't the main checkout,
   ...
   **Windows-specific recurring quirk (seen repeatedly under `.Codex/worktrees/agent-*` and
...
2. **WSL scratch clones** (see the `wsl-docker-linux-verification` project memory): a clone
   ...
   **Do not reach for `sudo`** (see "Commands needing a password/interactive auth" in
   `.Codex/rules/index.md`) — Docker itself already runs as root, so a second throwaway
...
5. **`.reference/<name>` — explicitly NOT debris, never remove.** Added 2026-09-12 (user's
   ...
   `AGENTS.md` "Локальные репозитории" and `.Codex/rules/index.md`), replacing the old
...
repo before (see `AGENTS.md`'s integrity-audit log, e.g. issue #218: `RequestCtx.
...
- **New code that's never actually wired in** — a config field that parses and validates but
  ...
  its chain (`FilterChain`/`ResponseFilterChain`/`build_handler()` — see `AGENTS.md`
