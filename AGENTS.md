...
- This applies to the *product* — not to internal maintainer notes. `AGENTS.md` itself and
  `.Codex/**` are the user's own operational tooling/notes and are written in the user's
...
> Note: this log previously existed only on the `Codex/cargo-workspace-features-23qxfr`
> migration branch's copy of `AGENTS.md` — `main` never had it. PR #178 brings it to `main`
...
- **Один PR** на ветке `Codex/focused-albattani-40372a` (commits: fix
...
- **Бенчмарк-тулинг (дешёвый, для будущих сессий)**: создан агент
  `.Codex/agents/benchmark-runner.md` (haiku) + команда `.Codex/commands/benchmark.md`
...
- **Worktree-guards** (после того как .Codex-тулинг дважды оказывался в эфемерной
  worktree-копии): правило «Worktree persistence» в `.Codex/rules/index.md` + `Stop`-хук
  в user-настройках (`<user-home>\.Codex\settings.json`, `shell: powershell`),
  аддитивно зеркалит worktree `.Codex/{agents,commands,skills,rules}` → main checkout
...
### Процессные правки (2026-06-13, не в git — `.Codex/`)
...
- **Лимит длины файла**: `rules/conventions.md` «Code quality» — мягкий лимит 400 строк,
  ...
  `.Codex/agents/architect.md`) за планом разбиения.
...
- **PR #204/#206/#208** — все 3 давних CRITICAL rust:S3776, отложенных
  ...
  при разборе: AGENTS.md 2026-06-13 назвал CC-79 функцию
