...
|---|---|---|---|
| 2026-08-17 ~18:16 (hourly cycle firing) | 0 open (confirmed directly via `search_pull_requests author:app/dependabot`) | Not individually reconciled this pass — `list_issues` hit a GitHub API rate limit right after the branch list came back, so the usual per-branch "does it have a merged/closed PR" check was skipped to conserve calls. Branch count 27 non-main (up from 22 logged 2026-08-12) — the +6 gain matches PRs #209-214 opened this same session (all still open, not yet merged: user has an explicit standing hold on merging any of them without go-ahead, unrelated to this reflex check). The other ~20 pre-existing branches (`phase-*`, `refactor/service-rs-phases`, `ci/wire-standard-feature-pipeline`, `security/v1.1.0-stabilize`, etc.) match names of PRs already logged as merged earlier in this file — consistent with the known "no auto-delete-head-branches setting" cause, not evidence of anything new/untracked. Flagging for the user's own cleanup script rather than deleting branches unprompted. | Session already has 6 open PRs (#209-214) awaiting the user's explicit merge go-ahead (security-engineer PASS posted on all) — nothing new to triage here beyond that pre-existing state. |
