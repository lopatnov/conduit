...
|---|---|---|---|
| 2026-08-22 ~10:45 (`/feature-workspace-cycle` daily firing) | 0 open (confirmed via `gh pr list`/branch cross-reference) | 0 (24 branches, unchanged count from the ~07:00 row below — no new branch created this pass; `docs/sonarcloud-s5659-false-positive` is new since 2026-08-12 but traces to a closed, not-orphaned PR, see notes) | Continued the ~07:00 row's SonarCloud thread: Quality Gate on `main` was failing `new_security_rating` D (required ≥A) — traced to `rust:S5659` (type VULNERABILITY, not the Hotspot #238 already fixed) on `extract_claims_unchecked`'s `insecure_decode` call, `src/filter/jwt.rs:277`, still OPEN since 2026-06-06. Audited all 3 production call sites (`check_jwt_extracting`, `check_shared_jwt_consumer` in auth.rs, `jwt_claims_from_session` in request_phase.rs) — each is only reachable after signature verification earlier in the same request; `security-engineer` independently confirmed (adversarial pass, no missed call site/config-drift/TOCTOU found). Resolved SonarCloud issue `AaAlsvdOLm9l5kt0_A2m` as `falsepositive` directly via MCP (`change_sonar_issue_status`) — confirmed live, Quality Gate flipped to green immediately, no CI re-run needed. First attempt wrapped this in a docs-only PR (#240, full call-site writeup in `CLAUDE.md`, matching the OSV-suppression precedent) — **user closed it as unwanted bureaucracy** ("не вижу никаких решений кроме болтологии") and redirected: fix real issues with actual code+PR+merge, tell them directly in chat for false positives instead of ceremony. Course-corrected: found 9 other pre-existing OPEN minor issues (`rust:S1612` closure→method-ref ×6, `rust:S8863` redundant `'static` ×2) via a fresh issue search, fixed all 9 for real (6 files), got CI fully green (29 checks) and `security-engineer` PASS at head `e537179`, merged as [PR #241](https://github.com/lopatnov/conduit/pull/241) (`main` tip `32eb3b0`). Migration branch synced after (`git merge origin/main`, merge commit `7cb85d3`) — 2 real conflicts (`src/config/parse.rs`, `src/util/log_writer.rs`): both files had been superseded by the #114 crate-extraction work into `conduit-config-core`/`conduit-core` facades, so kept the migration branch's extracted versions and applied the identical S1612/mechanical fixes at their new locations (`crates/conduit-config-core/src/format.rs`, `crates/conduit-core/src/util/log_writer.rs`) instead of losing them. Local `cargo check`/`cargo fmt` verification was blocked by a missing `cmake` on this dev machine (same pre-existing environment gap noted in PR #241's own description) — relied on CI (tracking PR #152) for the real verification; all 33 checks passed including `Feature isolation (cargo-hack)` and `Footprint report`. Process note for future firings: when a SonarCloud finding is a false positive, resolve it via the MCP tool and report it in chat — skip the docs-PR ceremony unless the user asks for a written record. |
