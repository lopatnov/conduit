...
### Реализовано в сессии 2026-09-25/26 (Pingora 0.9.0 на ветке миграции — PR #450; #314 — PR #460 смерджен; main по-прежнему заморожен)
...
- **Открыто на конец сессии:** #459 (отложено владельцем), #461 (токен в report-jobs), #451, #457, #444, #449, #447, #358, Sonar-находки, ~20 старых remote-веток; **следующий шаг — #315** (`validate.rs`: 1495 code lines, 36 `#[cfg]`-гейтов вместо 12 из плана архитектора на #222 — план по цифрам устарел, `-- --list` до/после и пруф-верификатор как в #314; заодно поправить `MAX_ATTR_LINES`), затем #316, #222, #145–#148.
