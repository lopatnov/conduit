...
#[test]
fn upload_file_exceeds_per_file_limit_leaves_no_partial_file_on_disk() {
    // Issue #277: before the streaming fix, a rejected oversized upload was
    // at least never partially written (the whole thing was buffered in
    // memory first, then the destination file was only ever created after
    // the size check passed). The streaming rewrite writes chunks to disk
    // as they arrive and only detects the violation partway through, so it
    // must now explicitly clean up the partial file on rejection instead of
    ...
    let client = build_client();
    let big_data = vec![b'x'; 5_000_000]; // well over the 10-byte limit
    let form = reqwest::blocking::multipart::Form::new().part(
        "file",
        reqwest::blocking::multipart::Part::bytes(big_data)
            .file_name("big.txt")
    ...
    // With true streaming (issue #277's fix), the server detects the
    // violation and responds with 413 as soon as it's read enough of the
    // field to know -- for a large enough body, that can happen *before*
    // the client has finished sending the rest, so the connection gets
    // closed out from under an in-progress send. Real HTTP clients
    // (reqwest here, but also e.g. curl) surface that as a transport-level
    // send error rather than a clean 413 response, not unlike how nginx's
    // `client_max_body_size` rejection is commonly observed as a connection
    // reset rather than a graceful response body for a large enough upload.
    // Both outcomes are a correctly-rejected upload; what actually matters
    // for this test is that no partial file survives on disk either way --
    // the server-side cleanup runs synchronously inside the request
    // handler before it returns, so it has already happened by the time
    // `send()` returns, whichever way it returns.
    match client.post(server.url("/upload")).multipart(form).send() {
        Ok(resp) => assert_eq!(resp.status(), 413, "file over maxFileSizeBytes → 413"),
        Err(e) => assert!(
