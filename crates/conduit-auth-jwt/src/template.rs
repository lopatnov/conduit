...
//!
//! Lives outside the `jwt`/`guard` modules (which require this crate's own
//! `jwt` Cargo feature) because the header-transform code path that calls
//! this is always compiled in the root crate: a config can reference
//! `{{ jwt.<claim> }}` syntax regardless of whether the `jwt` feature is
//! enabled. Claims are only ever populated by `#[cfg(feature = "jwt")]`-gated
//! code elsewhere (`guard::extract_claims_from_session`), so without that
//! feature `claims` is always `None` here and every template resolves to
//! `""` — matching the root crate's behavior before this module was
//! extracted (originally split out of the root crate's `filter::jwt` module
//! in #123 as `crate::util::jwt_template`, moved into this crate as
//! `crate::template` in #133).
//!
//! `pub` (not `pub(crate)`) because the root crate calls this across the
//! crate boundary — see the `pub(crate) use` facade at
//! `src/util/jwt_template.rs`, which re-narrows the visibility back to
//! `pub(crate)` on the root crate's side to preserve the original
