...
/// name — the same identity-spoofing shape `ConsumersGuard` guards against
/// for `X-Consumer-ID` (fixed on `main` via #344, ported here during the
/// migration branch's sync with `main`).
